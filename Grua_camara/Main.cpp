...
* Estructuras usadas
*/
strutSuelo baseSuelo = { -TAMSUELO, TAMSUELO, -TAMSUELO, TAMSUELO, 0.5f, {1.0f, 1.0f, 1.0f} };
...
esfera esferaEstrellas = { 0, 0,  0,  25.0f, 0.0f, {1.0f, 1.0f, 1.0f} };
...
char estrellas[14] = "caust/stm.png";
...
void suelo() 
{
	...
	for (float i = baseSuelo.X_neg; i < baseSuelo.X_pos; i += baseSuelo.intervalo)
	{
		for (float j = baseSuelo.Z_neg; j < baseSuelo.Z_pos; j += baseSuelo.intervalo)
		{
			glPushMatrix();
...
void charco() {
	glBindTexture(GL_TEXTURE_2D, baseSuelo.imagenes[agua]);
	glColor3f(1.0f, 1.0f, 1.0f);
...
boolean espacioProhibido(float X, float Z) {
	...
	/*else if (Z < TAMCHARCO + TAMCUBO && Z > -TAMCHARCO - TAMCUBO)
		return TRUE;*/
	else if (X > TAMSUELO || X < -TAMSUELO || Z > TAMSUELO || Z < -TAMSUELO)
		return TRUE;
	else return FALSE;
}
...
void inicializarImagenes() {
	for (int i = 0; i < FRAMESUELO; i++) {
		baseSuelo.imagenes[i] = Carga_Texturas(foto[i], 0);
	}
	esferaEstrellas.textura = Carga_Texturas(estrellas, 1);
	baseSuelo.textura = Carga_Texturas(hierba, 1);
